...
1. python里的for是不支持for j = i+1; j < len(nums); j += 1 这种写法的，只能 for x in iteration
2. python切片：list[0:3]  //注意这里不是list[0,3]
   >>> l = ['a','b','c','d']
   >>> l[0:3]
   ['a', 'b', 'c']
--------------------------------------------超时————————————————————————————————————————————————————————
    ...
    def maxSubArray(self, nums):
        ...
        for i in range(len(nums)):
            ...
            for j in range(i+1, len(nums)):
                if maxsum < self.sumfunc(nums[i:j+1]):
                    maxsum = self.sumfunc(nums[i:j+1])
        return maxsum
